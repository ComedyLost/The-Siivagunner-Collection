...
vec4 transverseChromatic(vec2 p) {
	...
	for (int i = 0; i < sampleCount; i++) {
		acc.r += texture2D(bitmap, destCoord + offsets1).r;
		acc.g += texture2D(bitmap, destCoord + offsets2).g;
		acc.b += texture2D(bitmap, destCoord + offsets3).b;
        offsets1 -= increment1;
        offsets2 -= increment2;
        offsets3 -= increment3;
	}

    vec4 newColor = vec4(acc / float(sampleCount), 1.0);
	return newColor;
}
...
void main() {
    ...
    vec4 texColor = texture2D(bitmap, p); // Sample full RGBA
    float alpha = texColor.a * openfl_Alphav;

    // Early exit for fully transparent pixels
    if (alpha < 0.01) {
        discard; // Avoid drawing black pixels
    }
    ...
    vec4 chroma = transverseChromatic(p);
    color = chroma.rgb;
